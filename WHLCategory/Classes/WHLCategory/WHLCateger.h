//
//  whl_Categer_pch.h
//  testOneHaveTImeYouCan
//
//  Created by 王浩霖 on 2020/6/13.
//  Copyright © 2020 HuiJuCha. All rights reserved.
//

#ifndef whl_Categer_pch_h
#define whl_Categer_pch_h

/*
 自己需要的使用的类可以在这里面进行添加和删除
 */

#import "UIView+WHLCustomAlertView.h"
#import "UIBarButtonItem+WHLCreate.h"
#import "UITextField+WHLPlaceholder.h"
#import "UIView+WHLNibLoad.h"
#import "UINavigationBar+WHLHandle.h"
#import "UITabBar+WHLTabBadgeView.h"
#import "UIView+WHLCornerRadius.h"
#import "NSString+WHLAES128.h"
#import "UIView+WHLFrame.h"
#import "UIColor+WHLTransform.h"
#import "NSDictionary+WHLCategory.h"
#import "NSString+WHLAdditions.h"
#import "NSDate+WHLCategory.h"
#import "NSString+WHLAppPath.h"
#import "UITextView+WHLPlaceholder.h"
#import "UITableView+WHLRegisterCell.h"
#import "UICollectionView+WHLRegisterCell.h"
#import "UIViewController+WHLBarItem.h"
#import "UIView+WHLNibLoad.h"
#import "UIView+WHLCustomAlertView.h"
#import "UITextField+WHLPlaceholder.h"
#import "UIBarButtonItem+WHLCreate.h"



#endif /* whl_Categer_pch_h */
