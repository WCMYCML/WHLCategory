...
static const void *const WHLCustomAlertViewKey = &WHLCustomAlertViewKey;

@implementation UIView (WHLCustomAlertView)

static WHLCustomAnimationMode mode;
...
- (void)setCQhideCustomAlertViewBlock:(dispatch_block_t)CQhideCustomAlertViewBlock {
    const void *key = (const void *)(WHLCustomAlertViewKey + 2);

    objc_setAssociatedObject(self, key, CQhideCustomAlertViewBlock,
                             OBJC_ASSOCIATION_COPY);
}

- (dispatch_block_t)CQhideCustomAlertViewBlock {
    const void *key = (const void *)(WHLCustomAlertViewKey + 2);
    dispatch_block_t hideCostomBlock = objc_getAssociatedObject(self, key);
    if (hideCostomBlock != nil) {
        return hideCostomBlock;
    }
    return nil;
}
...
- (void)whl_showInWindowWithMode:(WHLCustomAnimationMode)animationMode inView:(UIView *)superV bgAlpha:(CGFloat)alpha needEffectView:(BOOL)isNeed {
    ...
    switch (animationMode) {
        case WHLCustomAnimationModeAlert:
            [self showInWindow];
            break;
        case WHLCustomAnimationModeDrop:
            [self upToDownShowInWindow];
            break;
        case WHLCustomAnimationModeShare:
            [self shareViewShowInWindow];
            break;

        case WHLCustomAnimationModeNone:
...
- (void)clickBackView {
    if (self.CQhideCustomAlertViewBlock) {
        self.CQhideCustomAlertViewBlock();
        return;
    }
...
- (CGFloat)getAnimationTimeWithMode:(WHLCustomAnimationMode)type {
    switch (type) {
        case WHLCustomAnimationModeNone:
            return 0;
        case WHLCustomAnimationModeShare:
...
- (void)keyboardWillShow:(NSNotification *)noti {
    ...
    CGFloat space = mode == WHLCustomAnimationModeNone ? 0 : 10.0;
...
- (void)keyboardWillHide:(NSNotification *)noti {
    [UIView animateWithDuration:0.5 animations:^{
        if (mode == WHLCustomAnimationModeShare || mode == WHLCustomAnimationModeNone) {
            self.frame = CGRectMake(0, [UIApplication sharedApplication].keyWindow.bounds.size.height - self.frame.size.height, self.frame.size.width, self.frame.size.height);
        } else {
            self.center = [UIApplication sharedApplication].keyWindow.center;
        }
